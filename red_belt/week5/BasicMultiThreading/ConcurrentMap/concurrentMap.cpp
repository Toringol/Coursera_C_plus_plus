...
* (например, завернув наш map в шаблон Synchronized). 
* Теперь давайте представим, что у нас есть Synchronized<map<int, int>>, 
* в котором хранятся все ключи от 1 до 10000. Интуитивно кажется, 
* что когда из одного потока мы обращаемся к ключу 10, 
...
* Вам надо написать шаблон класса ConcurrentMap вот с таким интерфейсом:
...
* если ключ key присутствует в словаре, он должен возвращать объект класса Access, 
* содержащий ссылку на соответствующее ему значение; если же key отсутствует в словаре, 
* в него надо добавить пару (key, V()) и вернуть объект класса Access, 
...
public:
	...
	Access operator[](const K& key) {
		auto &seg = segments[submap_index(key)];
		return { lock_guard(seg.m), seg.submap[key] };
	}
	...
	map<K, V> BuildOrdinaryMap() {
		...
		for (auto& segment : segments) {
			lock_guard<mutex> locker(segment.m);
			mergedMap.insert(segment.submap.begin(), segment.submap.end());
		}
...
private:
	...
	size_t submap_index(K key) {
		return (abs(int(key)) % segments.size());
	}
