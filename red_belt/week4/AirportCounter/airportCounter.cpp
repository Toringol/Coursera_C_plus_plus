...
* 2) Набор возможных аэропортов фиксирован в виде enum и 
* потому не может изменяться в рамках одного сеанса работы системы статистики, 
* то есть в рамках одного экземпляра AirportCounter. Таким образом, 
* класс AirportCounter необходимо шаблонизировать типом этого enum: если назвать такой enum TAirport, 
* получим шаблонный класс AirportCounter<TAirport>.
* 3) Гарантируется (по крайней мере, обещано заказчиком), что по enum аэропортов 
* можно будет легко определить количество элементов в нём, а также перебрать сами элементы. 
...
* 4) Класс должен уметь конструироваться по списку аэропортов вылета, а также позволять обновлять этот список: 
* добавить вылет из какого-то аэропорта, удалить один вылет (например, ошибочно добавленный) 
* или удалить из истории все вылеты из некоторого аэропорта.
*/
...
public:
	...
	AirportCounter(TIterator begin, TIterator end) {
		while (begin != end) {
			airport_manager[static_cast<size_t>(*begin)]++;
			begin++;
		}
	}

	size_t Get(TAirport airport) const {
		return airport_manager.at(static_cast<size_t>(airport));
	}

	void Insert(TAirport airport) {
		airport_manager[static_cast<size_t>(airport)]++;
	}

	void EraseOne(TAirport airport) {
		if (airport_manager[static_cast<size_t>(airport)] != 0)
			airport_manager[static_cast<size_t>(airport)]--;
	}

	void EraseAll(TAirport airport) {
		airport_manager[static_cast<size_t>(airport)] = 0;
	}
	...
	Items GetItems() const {
		...
		for (auto& item : result) {
			...
			item.second = airport_manager.at(counter);
			counter++;
		}
		return result;
	}
...
private:
	array<size_t, static_cast<uint32_t>(TAirport::Last_)> airport_manager = { };
};
